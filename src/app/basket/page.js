...
export default function Basket() {
    ...
    return (
        <div className='w-full md:max-h-[554px] overflow-y-auto border border-neutral-400 rounded p-6'>
            <div className='flex flex-col gap-y-4'>
                {
                    basket.map((item, index) => {
                        return (
                            <div key={index} className='flex  border border-neutral-300 rounded-lg '>
                                <img src={item.image} className='w-[169px] bg-slate-600 h-auto' alt="" />

                                <div className='flex flex-col gap-y-1 py-4 px-8 w-full'>
                                    <div className='flex justify-between '>
                                        <span className='text-H7 font-SemiBold'>{item.title}</span>
                                        <div className=' cursor-pointer' onClick={() => {

                                            const removeItem = basket.filter((filterItem) => { return filterItem != item })
                                    ...
                                    <div>
                                        <span className='text-Body-SM font-Regular'>
                                            {item.deatails}
                                        </span>
                                    </div>
                                    <div className='flex  justify-between '>
